...
def get_chat_rag_agent(
    ...
    embedder = OllamaEmbedder(model=embeddings_model, dimensions=768) # 3072
    knowledge_base = PDFKnowledgeBase(
        path="/home/ari/Documents/Thesis/damathWebApp/agent/dataset/GameMechanics.pdf",
        vector_db=PgVector2(db_url=db_url, collection=f"agent_documents", embedder=embedder),
        reader=PDFReader(chunk=True),
    )
    knowledge_base.load(recreate=True)

    def convert_to_list(x: str) -> list:
        """
        Converts a string representation of a list to an actual list.

        Args:
            x: String representation of a list
    ...
    return Agent(
        ...
        instructions=[
            # "Provide  valid moves of red pieces in a given board state"
