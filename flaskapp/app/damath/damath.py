...
def create_conversation(gamehist):
    history = list(gamehist)
    history = format_partial(history, isPartial=False)
    history = history.split(",")
    input_convo = {"role": "user"}
    output_convo = {"role": "assistant"}
    convo_list = []

    move = history.pop(0)
    while history != []:
        new_input_convo = dict(input_convo)
        new_input_convo["content"] = move
        element = history.pop(0)
        new_output_convo = dict(output_convo)
        new_output_convo["content"] = element

        move = ",".join([move, element])
        convo_list.append(new_input_convo)
        convo_list.append(new_output_convo)
...
def format_partial(gamehist, isPartial=True):
    # Conversion logic
    formatted_moves = []
    for move in gamehist:
        player, positions, _ = move
        source = positions[0]
        destination = positions[-1]
        if len(positions) == 2:
            # Normal move
            formatted_moves.append(f"{player}{source}-{destination}")
        elif len(positions) > 2:
            # Capture move
            formatted_moves.append(f"{player}{source}x{destination}")
    if isPartial:
        move = formatted_moves.pop()
        # Combine all formatted moves into a single string, separated by commas
        formatted_string = ",".join(formatted_moves)
        if formatted_string == "":
            formatted_string = "START"
        return formatted_string, move
    else:
        isPartial = True
        # Combine all formatted moves into a single string, separated by commas
        formatted_string = ",".join(formatted_moves)
...
def write_partial(gamehist):
    if random.random() < 0.15:
        file_path = "partialhist_to_move.csv"
        file_exists = os.path.isfile(file_path) and os.path.getsize(file_path) > 0
        with open(file_path, "a", newline="") as file:
            new_gamehist, new_move = format_partial(gamehist)
            writer = csv.writer(file)

            if not file_exists:
                writer.writerow(["history", "move"])
            writer.writerow([
                new_gamehist,
                new_move
            ])
...
class Board:
    def __init__(self):
        ...
        # self.board = [
        # [[Piece('red', 2, is_dama=False), '*'], 'X', [Piece('red', -5, is_dama=False), '/'], 'X', [Piece('red', 8, is_dama=False), '-'], 'X', [Piece('red', -11, is_dama=False), '+'], 'X', 'X', [None, '/'], 'X', [Piece('red', 10, is_dama=True), '*'], 'X', [Piece('red', -3, is_dama=False), '+'], 'X', [None, '-'], [Piece('red', -7, is_dama=False), '-'], 'X', [Piece('red', -1, is_dama=False), '+'], 'X', [Piece('blue', -7, is_dama=False), '*'], 'X', [None, '/'], 'X', 'X', [None, '+'], 'X', [None, '-'], 'X', [None, '/'], 'X', [None, '*'], [None, '*'], 'X', [Piece('red', 4, is_dama=False), '/'], 'X', [Piece('blue', -3, is_dama=False), '-'], 'X', [None, '+'], 'X', 'X', [None, '/'], 'X', [None, '*'], 'X', [None, '+'], 'X', [None, '-'], [None, '-'], 'X', [Piece('blue', -9, is_dama=False), '+'], 'X', [Piece('blue', 10, is_dama=False), '*'], 'X', [None, '/'], 'X', 'X', [Piece('blue', -11, is_dama=False), '+'], 'X', [Piece('blue', 8, is_dama=False), '-'], 'X', [Piece('blue', -5, is_dama=False), '/'], 'X', [Piece('blue', 2, is_dama=False), '*']]
        # ]
...
class Game:
    def __init__(self):
        ...
        # self.board.board[48][0] = (Piece("b", 7))
        # self.board.board[48][0].index = 48
        # self.board.board[48][0].is_dama = True

        # self.board.board[13][0] = (Piece("r", 10))
        # self.board.board[13][0].index = 13
        ...
        # USE THIS
        x = """[[Piece('r', 2, is_dama=False), '*'], 'X', [Piece('r', -5, is_dama=False), '/'], 'X', [None, '-'], 'X', [Piece('r', -11, is_dama=False), '+'], 'X', 'X', [None, '/'], 'X', [Piece('r', 8, is_dama=False), '*'], 'X', [None, '+'], 'X', [Piece('r', 0, is_dama=False), '-'], [None, '-'], 'X', [None, '+'], 'X', [None, '*'], 'X', [None, '/'], 'X', 'X', [None, '+'], 'X', [None, '-'], 'X', [None, '/'], 'X', [Piece('b', -1, is_dama=False), '*'], [Piece('r', -7, is_dama=False), '*'], 'X', [None, '/'], 'X', [None, '-'], 'X', [None, '+'], 'X', 'X', [None, '/'], 'X', [Piece('r', -9, is_dama=False), '*'], 'X', [None, '+'], 'X', [Piece('b', 4, is_dama=False), '-'], [Piece('b', 0, is_dama=False), '-'], 'X', [None, '+'], 'X', [Piece('b', -5, is_dama=False), '*'], 'X', [Piece('b', -7, is_dama=False), '/'], 'X', 'X', [None, '+'], 'X', [None, '-'], 'X', [None, '/'], 'X', [Piece('b', 2, is_dama=False), '*']]"""
    ...
    def write_to_file(self):
        ...
        # if os.path.isfile("decision.csv") and os.path.getsize("decision.csv") == 0:
                ...
                for val in moves:
                    ...
                    if is_capture:
                        try:
                            ...
                            # Apply dama scoring multipliers
                            capturing_is_dama = key.is_dama
                            captured_is_dama = self.board.board[key.capture_index[val]][0].is_dama

                            if capturing_is_dama and captured_is_dama:
                                score *= 4  # Quadruple score for dama capturing dama
                            elif capturing_is_dama or captured_is_dama:
                                score *= 2  # Double score if either piece is dama
                        ...
                        # writer.writerow([self.board.to_json(), self.valid_moves_to_json(), decision])
                    else:
                        move_data = {
                            "move": {
                                "source": {
                    ...
                    #     valid_moves_str += f"{piece}: {moves}\n"
                    # writer.writerow([
                    #         self.board, self.valid_moves_to_json(), valid_moves_str.strip()
                    # ])
                    writer.writerow([json.loads(self.board.to_json())["board"], json.loads(self.valid_moves_to_json())["valid_moves"]])
        ...
        for i, val in enumerate(diagonals):
            ...
            if (
                ...
                if self.board.board[landing_squares[i]][0] == None:
                    ...
                    capture.append(landing_squares[i])
            ...
            while True:
                ...
                if isinstance(self.board.board[starting_index][0], Piece):
                    ...
                    if self.board.board[starting_index][0].color != self.current_move:
                        ...
                        dama_capture = self.dama_potential_capture(
                            piece, self.board.board[starting_index][0], direction
                        )

                        if dama_capture != ():
                            piece.capture_index[dama_capture] = starting_index
                            ...
                            dama_moves.append(tuple(dama_capture))
    ...
    def check_valid_moves(self, piece):
        ...
        capture = self.check_potential_capture(piece)
        # print(capture)
    ...
    def move(self, turn):
        ...
        while True:
                ...
                for key in piece_chosen[0].capture_index:
                    if piece_destination in key:
                        ...
                        try:
                            ...
                            # Apply dama scoring multipliers
                            capturing_is_dama = self.board.board[piece_chosen_index][0].is_dama
            ...
            # before checking the possibility of multiple capture,
            # the move is already performed.
            # call the write function here
            write_partial(self.move_history)
...
if not generate_dataset:
    ...
    for game_id in range(1, rows+1):
        ...
        with open("gamehistory.csv", "a", newline="") as file:
            ...
            if not file_exists:
                writer.writerow(["game_id", "conversations", "game_history", "red_score", "blue_score", "win"])
            writer.writerow([
                game_id,
                create_conversation(game.move_history),
                format_partial(game.move_history, isPartial=False),
                red,
                blue,
