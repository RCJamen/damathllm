...
def func1(board_state):
    ...
    for i, element in enumerate(board_state):
        if element == 'X':
            ...
            # Assign index to the Piece object
            if piece.color == 'r':
                piece.index = i

            # Call func2 and update valid_moves dictionary
        ...
        if dest_index < len(board_state):
            ...
            if isinstance(square, list):
                ...
                if first_element is None:
                  ...
                  isinstance(square, Piece) or
                  square in ['/', '-', '+']):
                continue

        # If the move is not within range of the board state, consider it invalid
