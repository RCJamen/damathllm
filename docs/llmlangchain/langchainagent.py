...
from langchain.agents import initialize_agent, create_react_agent, create_tool_calling_agent
...
class LLMManager:
    ...
    def initialize_llm(self, model_name: str = "llama3.2") -> bool:
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    ("system", "You are a helpful assistant. Always use the tools when needed, and return only the final tool output, NOT the tool call JSON."),
                    ("human", "{input}"),
            ...
            # tools = [get_current_time, get_red_valid_moves]
            ...
            self.agent = create_tool_calling_agent(self.llama, tools, prompt)
            self.agent = AgentExecutor(agent=self.agent, tools=tools, verbose=True, handle_parsing_errors=True)
            ...
            #     "input": "What's the current time? Use ONLY the provided tools and return ONLY the time as your final answer"
            # })
            ...
            response = self.agent.invoke({"input":"What is 3 + 4?"})
            ...
            if isinstance(response["output"], str):  # If output is JSON as a string
                tool_call = json.loads(response["output"])  # Convert to dictionary
                tool_name = tool_call["name"]
                tool_params = tool_call["parameters"]
                
                print("Tool Called:", tool_name)
                print("Tool Parameters:", tool_params)
            else:
                print("Final Output:", response["output"]) 

            # if tool_call["name"] == "get_current_time":
            #     actual_time = get_current_time.invoke({})
            #     print("Actual Current Time:", actual_time)
    ...
    def get_response(self, input: str) -> str:
        ...
        # self.agent = AgentExecutor(agent=self.agent, tools=tools, verbose=True)
