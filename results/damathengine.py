...
class Game:
    ...
    def is_game_over(self, player: str) -> bool:
        self.check_all_valid(player)
        if (not self.valid_moves) or all(not dests for dests in self.valid_moves.values()):
            for index, item in enumerate(self.board.board):
                if isinstance(item, list):
                    element = item[0]
                    if isinstance(element, Piece):
                        score = element.value * 2 if element.is_dama == True else element.value
                        if element.color == "b":
                            self.scores["b"] += score
                        elif element.color == "r":
                            self.scores["r"] += score
            print("\nGame Over!\n\n")
            return True
        return False
...
def minimax_move(state: Game):
    ...
    def utility(state: Game):
        """
        Exact utility at terminal states: +inf for MAX win, -inf for MIN win,
        or difference in piece counts or weighted piece values.
        """
        max_score = state.scores["r"]
        min_score = state.scores["b"]
        # win/loss
        ...
        if max_score < min_score:
            ...
            return 5 * (max_score - min_score)
        
        # Dapat diay ang return ani is if ang final score sa both players dako ang maximizer, return math.inf. Negative if daog ang minimizer
        # if final score dako ang maximizer:
        #     return math.inf
        # else:
        #     return -math.inf
    
    def heuristic(state: Game) -> float:
        """
        For non-terminal states -> the score difference between the maximizer and the minimizer
        """
        max_score = state.scores["r"]
        min_score = state.scores["b"]
        return max_score - min_score
    
    def count_pieces(state: Game, player: str):
        count = 0
        for item in state.board:
            if isinstance(item, list):
                if isinstance(item[0], Piece):
                    if item[0].color == player:
                        count += 1
    ...
    def minimax(state: Game, depth: int, alpha: float, beta: float, max_depth: int, last_move: str) -> float:

        # if last_move == state.current_move:
        #     depth -= 1
        # Terminal by rules
        if state.is_game_over(state.current_move):
            return utility(state)
        # Depth cutoff
        if depth >= max_depth:
            return heuristic(state)

        last_move = state.current_move
        if state.current_move == maximizer:
            ...
            for move in move_list:
                ...
                child = new_state
                value = max(value, minimax(child, depth+1, alpha, beta, max_depth, last_move))
            ...
            for move in move_list:
                ...
                new_state.api_move(move[0], move[1])
                child = new_state
                value = min(value, minimax(child, depth+1, alpha, beta, max_depth, last_move))
                beta = min(beta, value)
                if beta <= alpha:
                    break  # alpha cutoff
    ...
    def choose_best_move(state: Game, max_depth: int):
        ...
        for move in move_list:
            ...
            new_state.api_move(move[0], move[1])
            child = new_state
            val = minimax(child, 1, -math.inf, math.inf, max_depth, state.current_move)
            if (state.current_move==maximizer and val > best_value) or \
            (state.current_move==minimizer and val < best_value):
                best_value = val
                best_move = move
        return best_move[0], best_move[1]
    
    return choose_best_move(state_copy, max_depth)