...
from app.engine.models import FIGURE_REPR, Color, Figure, SquareT
from app.engine.move import Move
...
class BaseBoard(ABC):
    ...
    **Structure:**
    ...
    def push(self, move: Move, is_finished: bool = True) -> None:
        ...
        if (
            (tg // (self.shape[0] // 2)) == 0
            and self._pos[tg] == Figure.BLUE_MAN.value
            and is_finished
        ) or (
            (tg // (self.shape[0] // 2)) == (self.shape[0] - 1)
            and self._pos[tg] == Figure.RED_MAN.value
            and is_finished
        ):
    ...
    def from_fen(cls, fen: str) -> BaseBoard:
        ...
        re_blue = re.compile(r"W[0-9K,]+")
        re_red = re.compile(r"B[0-9K,]+")
        ...
        try:
            ...
            blue = re_blue.search(fen).group(0).replace("W", "")
            red = re_red.search(fen).group(0).replace("B", "")
        except AttributeError as e:
            raise AttributeError(f"Invalid FEN: {fen} \n {e}")
        logger.debug(f"turn: {turn}, blue: {blue}, red: {red}")
        cls.STARTING_POSITION = np.zeros(cls.STARTING_POSITION.shape, dtype=np.int8)
        if len(turn) != 1 or (len(blue) == 0 and len(red) == 0):
            raise ValueError(f"Invalid FEN: {fen}")
        try:
            cls.__populate_from_list(blue.split(","), Color.BLUE)
            cls.__populate_from_list(red.split(","), Color.RED)
        except ValueError as e:
            logger.error(f"Invalid FEN: {fen} \n {e}")
        turn = Color.BLUE if turn == "B" else Color.RED
    ...
    def result(self) -> Literal["1/2-1/2", "1-0", "0-1", "-"]:
        ...
        if self.is_draw:
            return "1/2-1/2"
        if self.turn == Color.BLUE and self.game_over:
            return "0-1"
        if self.turn == Color.RED and self.game_over:
            return "1-0"
        return "-"
