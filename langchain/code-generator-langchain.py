...
normal_moves_instruction = f"""I have an initial board state (A 1-dimensional array indicated as a Python list).
...
dama_moves_instruction = f"""I have an initial board state (A 1-dimensional array indicated as a Python list).
I want you to create a code that takes in a board state as an input with the function name 'func1'.
...
normal_captures_instruction = f"""I have an initial board state (A 1-dimensional array indicated as a Python list).
...
dama_captures_instruction = f"""I have an initial board state (A 1-dimensional array indicated as a Python list).
...
def generate_normal_moves():
    print("Generating normal moves...")
    normal_moves = normal_moves_runLLM(normal_moves_instruction)
    normal_moves = normal_moves.content
    formatOutput(normal_moves, "normal_moves")
    time.sleep(10)

def generate_dama_moves():
    print("Generating dama moves...")
    dama_moves = dama_moves_runLLM(dama_moves_instruction)
    dama_moves = dama_moves.content
    formatOutput(dama_moves, "dama_moves")
    time.sleep(10)

def generate_normal_captures():
    print("Generating normal captures...")
    normal_captures = normal_captures_runLLM(normal_captures_instruction)
    normal_captures = normal_captures.content
    formatOutput(normal_captures, "normal_captures")
    time.sleep(10)

def generate_dama_captures():
    print("Generating dama captures...")
    dama_captures = dama_captures_runLLM(dama_captures_instruction)
    dama_captures = dama_captures.content
    formatOutput(dama_captures, "dama_captures")

def main():
    start = time.time()

    generators = {
        "normal_moves": generate_normal_moves,
        "dama_moves": generate_dama_moves,
        "normal_captures": generate_normal_captures,
        "dama_captures": generate_dama_captures
    }
    ...
    if len(sys.argv) == 1:
        ...
        if generator_type in generators:
            print(f"Running {generator_type} generator...")
            generators[generator_type]()
        else:
            print(f"Unknown generator type: {generator_type}")
            print("Available types:", list(generators.keys()))
    else:
        print("Usage: python3 code-generator-langchain.py [normal_moves|dama_moves|normal_captures|dama_captures]")
        print("If no argument is provided, all generators will run.")
